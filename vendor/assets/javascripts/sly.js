...
(function (k, B, Ba) {
    function la(f, h, Va) {
        ...
        function aa() {
            ...
            if (C) {
                ...
                F.each(function (b, l) {
                    ...
                    b === r && (h.end += ta, t += ta, s = p ? d : 0);
                    n.push(h);
                    z = h
                });
                u ? (u[0].style[c.horizontal ? "width" : "height"] = (h ? t : t -
                    a - ta) + "px") : null
                t -= s;
            ...
            D.length && 0 < S && (c.dynamicHandle ? (N = e.start === e.end ? S : Math.round(S * y / t), N = v(N, c.minHandleSize, S), D[0].style[c.horizontal ? "width" : "height"] = N + "px") : N = D[c.horizontal ? "outerWidth" : "outerHeight"](), q.end = S - N, U || Ca());
            if (!H && 0 < y) {
                var p = e.start,
                    h = "";
                if (C) k.each(n, function (b, l) {
                    ...
                    for (; p - y < e.end;) w.push(p), p += y; if (V[0] && l !== w.length) {
            ...
            g.sbSize = S;
            g.handleSize = N;
            C ? (d.initialized ? (g.activeItem >= n.length || 0 === b && 0 < n.length) && oa(g.activeItem >= n.length ? n.length - 1 : 0, !b) : (oa(c.startAt), d[L ? "toCenter" : "toStart"](c.startAt)), I(L && n.length ? n[g.activeItem].center : v(e.dest, e.start, e.end))) :
                d.initialized ? I(v(e.dest, e.start, e.end)) : I(c.startAt, 1);
            A("load")
        }
        ...
        function I(b, l, fa) {
            ...
            k.extend(g, ua(void 0));
            Da();
            ga[0] && r.page !== g.activePage && (r.page = g.activePage, ga.removeClass(c.activeClass).eq(g.activePage).addClass(c.activeClass), A("activePage", r.page))
        }
        ...
        function va() {
            U ? (sa ? e.cur = s : B ? (ma = s - e.cur, 0.1 > Math.abs(ma) ? e.cur = s : e.cur += ma * (a.released ? c.swingSpeed : c.syncSpeed)) : (R = Math.min(+new Date - z, c.speed), e.cur = qa + ra * jQuery.easing[c.easing](R / c.speed, R, 0, 1, c.speed)), s === e.cur ? (e.cur = s, a.tweese = U = 0) : U = ha(va), A("move"), H || (J ?
                u[0].style[J] = ia + (c.horizontal ? "translateX" : "translateY") + "(" + -e.cur + "px)" : u[0].style[c.horizontal ? "left" : "top"] = -Math.round(e.cur) + "px"), !U && a.released && A("moveEnd"), Ca()) : (U = ha(va), a.released && A("moveStart"))
        }

        function Ca() {
            D.length && (q.cur = e.start === e.end ? 0 : ((a.init && !a.slidee ? e.dest : e.cur) - e.start) / (e.end - e.start) * q.end, q.cur = v(Math.round(q.cur), q.start, q.end), r.hPos !== q.cur && (r.hPos = q.cur, J ? D[0].style[J] = ia + (c.horizontal ? "translateX" : "translateY") + "(" + q.cur + "px)" : D[0].style[c.horizontal ? "left" :
                "top"] = q.cur + "px"))
        }
        ...
        function wa(b, l, a) {
            "boolean" === ja(l) && (a = l, l = Ba);
            l === Ba ? I(e[b], a) : L && "center" !== b || (l = d.getPos(l)) && I(l[b], a, !L)
        }
        ...
        function oa(b, l) {
            var a =
                pa(b);
            if (!C || 0 > a) return !1;
            if (r.active !== a || l) F.eq(g.activeItem).removeClass(c.activeClass), F.eq(a).addClass(c.activeClass), r.active = g.activeItem = a, Da(), A("active", a);
            return a
        }
        ...
        function Da() {
            var b = e.dest <= e.start,
                l = e.dest >= e.end,
                d = b ? 1 : l ? 2 : 3;
            r.slideePosState !== d && (r.slideePosState = d, W.is("button,input") && W.prop("disabled", b), X.is("button,input") && X.prop("disabled", l), W.add(ba)[b ? "addClass" : "removeClass"](c.disabledClass), X.add(Y)[l ? "addClass" : "removeClass"](c.disabledClass));
            r.fwdbwdState !== d && a.released && (r.fwdbwdState = d, ba.is("button,input") &&
                ba.prop("disabled", b), Y.is("button,input") && Y.prop("disabled", l));
            C && (b = 0 === g.activeItem, l = g.activeItem >= n.length - 1, d = b ? 1 : l ? 2 : 3, r.itemsButtonState !== d && (r.itemsButtonState = d, Z.is("button,input") && Z.prop("disabled", b), $.is("button,input") && $.prop("disabled", l), Z[b ? "addClass" : "removeClass"](c.disabledClass), $[l ? "addClass" : "removeClass"](c.disabledClass)))
        }
        ...
        function Ga(b, a, d) {
            ...
            if (-1 < b && -1 < a && b !== a && !(d && a === b - 1 || !d && a === b + 1)) {
                ...
                var c = b < a ? b :
                    ...
                    e = b > a ? b : d ? a + 1 : a,
                    f = b > a;
                b === g.activeItem ? r.active = g.activeItem = d ? f ? a + 1 : a : f ? a : a - 1 : g.activeItem > c && g.activeItem < e && (r.active = g.activeItem += f ? 1 : -1);
                aa()
            }
        }
        ...
        function Ia(b) {
            return Math.round(v(b, q.start, q.end) / q.end * (e.end - e.start)) + e.start
        }
        ...
        function Ka(b) {
            if (!(a.init || ~k.inArray(b.target.nodeName, La) || k(b.target).is(c.interactive))) {
                var l = "touchstart" === b.type,
                    f = b.data.source,
                    g = "slidee" === f;
                if ("handle" !== f || c.dragHandle && q.start !== q.end)
                    if (!g || (l ? c.touchDragging : c.mouseDragging && 2 > b.which)) l || M(b, 1), Ja(f), a.init = 1, a.$source = k(b.target), a.touch = l, a.pointer = l ? b.originalEvent.touches[0] : b, a.initX = a.pointer.pageX, a.initY = a.pointer.pageY, a.initPos = g ? e.cur : q.cur, a.start = +new Date, a.time = 0, a.path = 0, a.delta = 0, a.locked = 0, a.history = [0, 0, 0, 0], a.pathToLock = g ? l ? 30 : 10 : 0, a.initLoc =
                        a[c.horizontal ? "initX" : "initY"], a.deltaMin = g ? -a.initLoc : -q.cur, a.deltaMax = g ? document[c.horizontal ? "width" : "height"] - a.initLoc : q.end - q.cur, ca.on(l ? Ma : Na, Oa), d.pause(1), (g ? u : D).addClass(c.draggedClass), A("moveStart"), g && (Pa = setInterval(Wa, 10))
            }
        }
        ...
        function Oa(b) {
            ...
            if (!a.locked && a.path > a.pathToLock)
                ...
                else if (a.slidee) a.$source.on(da, Qa);
            a.released ? (a.touch || M(b), clearInterval(Pa), ca.off(a.touch ? Ma : Na, Oa), (a.slidee ? u : D).removeClass(c.draggedClass), d.resume(1), e.cur === e.dest && a.init && A("moveEnd"), a.init = 0, c.releaseSwing && a.slidee && (a.swing = 300 * ((a.delta - a.history[0]) / 40), a.delta += a.swing,
                a.tweese = 10 < Math.abs(a.swing))) : !a.locked && a.touch || M(b);
            I(a.slidee ? Math.round(a.initPos - a.delta) : Ia(a.initPos + a.delta))
        }
        ...
        function Xa(b) {
            ...
            ya = +new Date;
            x.last < ya - x.resetTime && (x.delta = 0);
            x.last = ya;
            x.delta += x.curDelta;
        ...
        function Ya(b) {
            c.scrollBy && e.start !== e.end && (M(b, 1), d.slideBy(c.scrollBy * Xa(b.originalEvent)))
        }

        function Za(b) {
            c.clickBar && b.target === K[0] && (M(b), I(Ia((c.horizontal ? b.pageX - K.offset().left : b.pageY - K.offset().top) - N / 2)))
        }
        ...
        function $a(b) {
            if (c.keyboardNavBy) switch (b.which) {
                case c.horizontal ? 37:
                    38 : M(b);
                    d["pages" === c.keyboardNavBy ? "prevPage" :
                        "prev"]();
                    break;
        ...
        function ab(b) {~
            k.inArray(this.nodeName, La) || k(this).is(c.interactive) ? b.stopPropagation() : this.parentNode === u[0] && d.activate(this)
        }
        ...
        function A(b, a) {
            if (G[b]) {
                za = G[b].length;
                for (P = Aa.length = 0; P < za; P++) Aa.push(G[b][P]);
                for (P = 0; P < za; P++) Aa[P].call(d, b, a)
            }
        }
        var c = k.extend({},
                la.defaults, h),
        ...
        var T = "forceCentered" === c.itemNav,
            ...
            db = c.dragSource ? k(c.dragSource) : E,
            Y = k(c.forward),
            ba = k(c.backward),
            Z = k(c.prev),
            $ =
        ...
        d.getPos = function (b) {
            ...
            var a = u.find(b).eq(0);
            return a[0] ? (b = c.horizontal ? a.offset().left - u.offset().left : a.offset().top - u.offset().top, a = a[c.horizontal ? "outerWidth" : "outerHeight"](), {
                start: b,
                center: b - y / 2 + a / 2,
                end: b - y + a,
                size: a
        ...
        d.moveBy = function (b) {
            p.speed = b;
            !a.init && (p.speed && e.cur !== (0 < p.speed ? e.end : e.start)) && (p.lastTime = +new Date, p.startPos = e.cur, Ja("button"), a.init = 1, A("moveStart"), ka(Fa), Ea())
        };
        d.stop = function () {
        ...
        d.add = function (b, a) {
            var d = k(b);
            C ? (null != a && n[0] ? n.length && d.insertBefore(n[a].el) : d.appendTo(u), a <= g.activeItem && (r.active = g.activeItem += d.length)) : u.append(d);
            aa()
        };
        d.remove = function (b) {
            if (C) {
                if (b = xa(b), -1 < b) {
                    ...
                    b < g.activeItem && (r.active = --g.activeItem);
                    aa();
                    a && (r.active = null, d.activate(g.activeItem))
                }
            } else k(b).remove(), aa()
        };
        ...
        d.on = function (b, a) {
            if ("object" === ja(b))
                ...
                c = b.split(" ");
                for (var e = 0, f = c.length; e < f; e++) G[c[e]] = G[c[e]] || [], -1 === Ha(c[e], a) && G[c[e]].push(a)
            } else if ("array" === ja(a))
        ...
        d.off = function (a, c) {
            if (c instanceof Array)
                ...
                for (var e = a.split(" "), f = 0,
                         g = e.length; f < g; f++)
                    if (G[e[f]] = G[e[f]] || [], null == c) G[e[f]].length = 0;
                    else {
                        var h = Ha(e[f], c); - 1 !== h && G[e[f]].splice(h, 1)
                    }
        };
        d.destroy = function () {
            ca.add(Sa).add(D).add(K).add(V).add(Y).add(ba).add(Z).add($).add(W).add(X).unbind("." + m);
        ...
        d.init = function () {
            if (!d.initialized) {
                ...
                if (c.backward) ba.on(Ta, ea);
                if (c.prev) Z.on(da, ea);
                if (c.next) $.on(da, ea);
                if (c.prevPage) W.on(da, ea);
                if (c.nextPage) X.on(da, ea);
                Sa.on("DOMMouseScroll." + m + " mousewheel." + m, Ya);
                if (K[0]) K.on(da, Za);
                if (C && c.activateOn) E.on(c.activateOn + "." + m, "*", ab);
                if (V[0] && c.activatePageOn) V.on(c.activatePageOn +
                    "." + m, "*", bb);
                db.on(Ua, {
                    source: "slidee"
                }, Ka);
                ...
                ca.bind("keydown." + m, $a);
                H || (E.on("mouseenter." + m + " mouseleave." + m, cb), E.on("scroll." + m, eb));
                aa();
    ...
    var m = "sly",
        ...
        Na = "mousemove." + m + " mouseup." + m,
        Ma = "touchmove." + m + " touchend." + m,
        da = "click." + m,
        Ta = "mousedown." + m,
        La = ["INPUT",
